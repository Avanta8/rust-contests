...
static mut RAND_SEED: Option<u64> = None;

pub fn random() -> &'static mut Random {
    unsafe {
        if let Some(rand) = RAND.as_mut() {
            rand
        } else {
            RAND = Some(Random::new(rand_seed()));
            random()
        }
    }
}
...
fn rand_seed() -> u64 {
    unsafe {
        if let Some(&seed) = RAND_SEED.as_ref() {
            seed
        } else {
            RAND_SEED = Some(
                (SystemTime::UNIX_EPOCH.elapsed().unwrap().as_nanos() & 0x59A60FBFE3A60E00) as u64,
            );
            rand_seed()
        }
    }
...
pub fn set_seed(seed: u64) {
    unsafe {
        RAND_SEED = Some(seed & 0x59A60FBFE3A60E00);
        RAND = Some(Random::new(rand_seed()));
    }
}
