...
impl Str<'static> {
    pub fn new() -> Self {
        Str::Vec(Vec::new(), PhantomData)
    }

    pub fn with_capacity(cap: usize) -> Self {
        Str::Vec(Vec::with_capacity(cap), PhantomData)
    }
}
...
impl Clone for Str<'_> {
    fn clone(&self) -> Self {
        match self {
            Str::String(s, _) => Str::Vec(Vec::from(s.as_bytes()), PhantomData),
            Str::Str(s) => Str::Vec(Vec::from(s.as_bytes()), PhantomData),
            Str::Vec(s, _) => Str::Vec(s.clone(), PhantomData),
            Str::U8(s) => Str::Vec(Vec::from(*s), PhantomData),
            Str::StringRef(s) => Str::Vec(Vec::from(s.as_bytes()), PhantomData),
            Str::VecRef(s) => Str::Vec(s.to_vec(), PhantomData),
        }
    }
}
